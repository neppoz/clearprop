...
<script>
    $(document).ready(function () {
        $('input[name="updateRead"]').change(function () {
            let dropdown_id = ($(this).parents('div').attr('id'));
            let user_alert_radio_value = ($(this).val());

            $.ajax({
                url: "{{ route('admin.user-alerts.updateRead') }}?user_alert_id=" + user_alert_radio_value,
                method: 'GET',
                success: function (data) {
                    let divToHide = $('#' + dropdown_id);
                    if (data > 0) {
                        divToHide.hide();
