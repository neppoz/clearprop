            ...
            <form method="POST" action="{{ route("admin.planes.update", [$plane->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('callsign') ? 'is-invalid' : '' }}" type="text"
                           name="callsign" id="callsign" value="{{ old('callsign', $plane->callsign) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('vendor') ? 'is-invalid' : '' }}" type="text"
                           name="vendor"
                           id="vendor" value="{{ old('vendor', $plane->vendor) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('model') ? 'is-invalid' : '' }}" type="text" name="model"
                           id="model" value="{{ old('model', $plane->model) }}">
                    @if($errors->has('model'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('prodno') ? 'is-invalid' : '' }}" type="text"
                           name="prodno"
                           id="prodno" value="{{ old('prodno', $plane->prodno) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('counter_type') ? 'is-invalid' : '' }}"
                            name="counter_type"
                        ...
                        @foreach(App\Plane::COUNTER_TYPE_SELECT as $key => $label)
                            <option
                                    value="{{ $key }}" {{ old('counter_type', $plane->counter_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    ...
                    @foreach(App\Plane::WARMUP_TYPE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('warmup_type') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="warmup_type_{{ $key }}" name="warmup_type"
                                   value="{{ $key }}" {{ old('warmup_type', $plane->warmup_type) === (string) $key ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="active" id="active"
                               value="1" {{ $plane->active || old('active', 0) === 1 ? 'checked' : '' }}>
