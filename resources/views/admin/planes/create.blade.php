    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.planes.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('callsign') ? 'is-invalid' : '' }}" type="text" name="callsign" id="callsign" value="{{ old('callsign', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vendor') ? 'is-invalid' : '' }}" type="text" name="vendor" id="vendor" value="{{ old('vendor', 'Custom') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prodno') ? 'is-invalid' : '' }}" type="text" name="prodno" id="prodno" value="{{ old('prodno', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('counter_type') ? 'is-invalid' : '' }}" name="counter_type" id="counter_type" required>
                    <option value disabled {{ old('counter_type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Plane::COUNTER_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('counter_type', '100') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Plane::WARMUP_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('warmup_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="warmup_type_{{ $key }}" name="warmup_type" value="{{ $key }}" {{ old('warmup_type', '0') === (string) $key ? 'checked' : '' }}>
