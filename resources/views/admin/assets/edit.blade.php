            ...
            <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($categories as $id => $category)
                            <option
                                value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $asset->category->id ?? '') == $id ? 'selected' : '' }}>{{ $category }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('plane') ? 'is-invalid' : '' }}" name="plane_id"
                        ...
                        @foreach($planes as $id => $plane)
                            <option
                                value="{{ $id }}" {{ (old('plane_id') ? old('plane_id') : $asset->plane->id ?? '') == $id ? 'selected' : '' }}>{{ $plane }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('plane'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('start_hours') ? 'is-invalid' : '' }}" type="number"
                           name="start_hours" id="start_hours" value="{{ old('start_hours', $asset->start_hours) }}"
                           step="1" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text"
                           name="start_date" id="start_date" value="{{ old('start_date', $asset->start_date) }}">
                ...
                <div class="form-group">
                    <label for="end_hours">{{ trans('cruds.asset.fields.end_hours') }}</label>
                    <input class="form-control {{ $errors->has('end_hours') ? 'is-invalid' : '' }}" type="number"
                           name="end_hours" id="end_hours" value="{{ old('end_hours', $asset->end_hours) }}" step="1">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text"
                           name="end_date" id="end_date" value="{{ old('end_date', $asset->end_date) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($statuses as $id => $status)
                            <option
                                value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $asset->status->id ?? '') == $id ? 'selected' : '' }}>{{ $status }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($locations as $id => $location)
                            <option
                                value="{{ $id }}" {{ (old('location_id') ? old('location_id') : $asset->location->id ?? '') == $id ? 'selected' : '' }}>{{ $location }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes"
                              id="notes">{{ old('notes', $asset->notes) }}</textarea>
                    @if($errors->has('notes'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('assigned_to') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($assigned_tos as $id => $assigned_to)
                            <option
                                value="{{ $id }}" {{ (old('assigned_to_id') ? old('assigned_to_id') : $asset->assigned_to->id ?? '') == $id ? 'selected' : '' }}>{{ $assigned_to }}</option>
                        @endforeach
                    </select>
